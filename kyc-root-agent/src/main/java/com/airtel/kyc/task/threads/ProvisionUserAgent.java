...
public class ProvisionUserAgent implements Runnable, DisposableBean {

	private static Logger logger = Logger.getLogger(ProvisionUserAgent.class);
	...
	public ProvisionUserAgent() {
	}

	public ProvisionUserAgent(TaskManagementService appservice, AppConstants appconstants) {
		this.appservice = appservice;
		this.appconstants = appconstants;
	}

	public ProvisionUserAgent(TaskManagementService appservice, AppConstants appconstants, int repeatCount) {
		this.appservice = appservice;
		this.appconstants = appconstants;
	...
	public void run() {
		...
		while (!Thread.currentThread().isInterrupted()) {
			...
			agentlog.setAgent_id(TaskEnum.PROVISION_USER.getTaskCode());
			try {
				if ("true".equals(appconstants.sv_validation_enabled)) {
				...
				if (repeatCount == 0)
					break;
				Thread.sleep(appconstants.APPLICATION_THREAD_PROVISION_USER_SLEEP);
