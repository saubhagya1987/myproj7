...
public class SVStatusAgent implements Runnable, DisposableBean {

	private static Logger logger = Logger.getLogger(SVStatusAgent.class);
	...
	public SVStatusAgent() {
	}

	public SVStatusAgent(TaskManagementService appservice, AppConstants appconstants) {
		this.appservice = appservice;
		this.appconstants = appconstants;
	}

	public SVStatusAgent(TaskManagementService appservice, AppConstants appconstants, int repeatCount) {
		this.appservice = appservice;
		this.appconstants = appconstants;
	...
	public void run() {
		...
		while (!Thread.currentThread().isInterrupted()) {
			...
			agentlog.setAgent_id(TaskEnum.SV_STATUS_UPDATION.getTaskCode());
			try {
				if ("true".equals(appconstants.sv_status_activate_enabled)) {
				...
				if (repeatCount == 0)
					break;
				Thread.sleep(appconstants.THREAD_NAME_SV_STATUS_UPDATION_SLEEP);
