...
public class EMAUnbarForBulkAgent implements Runnable, DisposableBean {

	private static Logger logger = Logger.getLogger(EMAUnbarForBulkAgent.class);
	...
	public EMAUnbarForBulkAgent() {
	}

	public EMAUnbarForBulkAgent(TaskManagementService appservice, AppConstants appconstants) {
		this.appservice = appservice;
		this.appconstants = appconstants;
	}

	public EMAUnbarForBulkAgent(TaskManagementService appservice, AppConstants appconstants, int repeatCount) {
		this.appservice = appservice;
		this.appconstants = appconstants;
	...
	public void run() {
		...
		while (!Thread.currentThread().isInterrupted()) {
			...
			agentlog.setAgent_id(TaskEnum.EMA_UNBARING_FOR_BULK.getTaskCode());
			try {
				if ("true".equals(appconstants.ema_unbar_bulk_enabled)) {
				...
				if (repeatCount == 0)
					break;
				//Thread.sleep(Long.parseLong(appconstants.unbarr_sleep));
				Thread.sleep(appconstants.APPLICATION_THREAD_EMA_UNBAR_BULK_SLEEP);
