...
public class EMAUnbarAgent implements Runnable, DisposableBean {

	private static Logger logger = Logger.getLogger(EMAUnbarAgent.class);
	...
	public EMAUnbarAgent() {
	}

	public EMAUnbarAgent(TaskManagementService appservice, AppConstants appconstants) {
		this.appservice = appservice;
		this.appconstants = appconstants;
	}

	public EMAUnbarAgent(TaskManagementService appservice, AppConstants appconstants, int repeatCount) {
		this.appservice = appservice;
		this.appconstants = appconstants;
	...
	public void run() {
		...
		while (!Thread.currentThread().isInterrupted()) {
			...
			agentlog.setAgent_id(TaskEnum.EMA_UNBARING.getTaskCode());
			try {
				if ("true".equals(appconstants.ema_unbar_enabled)) {
				...
				Thread.sleep(appconstants.APPLICATION_THREAD_EMA_UNBAR_SLEEP);
