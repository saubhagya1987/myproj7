...
public class HPUnbarAgent implements Runnable, DisposableBean {

	private static Logger logger = Logger.getLogger(HPUnbarAgent.class);
	...
	public HPUnbarAgent(TaskManagementService appservice, AppConstants appconstants) {
		this.appservice = appservice;
		this.appconstants = appconstants;
	}

	public HPUnbarAgent(TaskManagementService appservice, AppConstants appconstants, int repeatCount) {
		this.appservice = appservice;
		this.appconstants = appconstants;
	...
	public void run() {
		...
		while (!Thread.currentThread().isInterrupted()) {
			...
			agentlog.setAgent_id(TaskEnum.HP_UNBARING.getTaskCode());
			try {
				if ("true".equals(appconstants.hp_unbar_enabled)) {
				...
				if (repeatCount == 0)
					break;
				Thread.sleep(appconstants.APPLICATION_THREAD_HP_UNBAR_SLEEP);
