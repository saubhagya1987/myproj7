...
public class SVIMSIFetchAgent implements Runnable, DisposableBean {

	private static Logger logger = Logger.getLogger(SVIMSIFetchAgent.class);
	...
	public SVIMSIFetchAgent() {
	}

	public SVIMSIFetchAgent(TaskManagementService appservice, AppConstants appconstants) {
		this.appservice = appservice;
		this.appconstants = appconstants;
	}

	public SVIMSIFetchAgent(TaskManagementService appservice, AppConstants appconstants, int repeatCount) {
		this.appservice = appservice;
		this.appconstants = appconstants;
	...
	public void run() {
		...
		while (!Thread.currentThread().isInterrupted()) {
			...
			agentlog.setAgent_id(TaskEnum.SV_IMSI_FETCH.getTaskCode());
			try {
				if ("true".equals(appconstants.sv_imsi_fetch_enabled)) {
				...
				if (repeatCount == 0)
					break;
				Thread.sleep(appconstants.APPLICATION_THREAD_SV_VALIDATION_SLEEP);
