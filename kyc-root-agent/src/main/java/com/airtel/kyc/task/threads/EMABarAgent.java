...
public class EMABarAgent implements Runnable, DisposableBean {

	private static Logger logger = Logger.getLogger(EMABarAgent.class);
	...
	public EMABarAgent(TaskManagementService appservice, AppConstants appconstants) {
		this.appservice = appservice;
		this.appconstants = appconstants;
	}

	public EMABarAgent(TaskManagementService appservice, AppConstants appconstants, int repeatCount) {
		this.appservice = appservice;
		this.appconstants = appconstants;
	...
	public void run() {
		...
		while (!Thread.currentThread().isInterrupted()) {
			...
			agentlog.setAgent_id(TaskEnum.EMA_BARING.getTaskCode());
		
			try {
				if ("true".equals(appconstants.ema_bar_enabled)) {
				...
				Thread.sleep(appconstants.APPLICATION_THREAD_EMA_BAR_SLEEP);
