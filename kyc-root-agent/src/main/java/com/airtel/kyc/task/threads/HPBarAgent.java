...
public class HPBarAgent implements Runnable, DisposableBean {

	private static Logger logger = Logger.getLogger(HPBarAgent.class);
	...
	public HPBarAgent(TaskManagementService appservice, AppConstants appconstants) {
		this.appservice = appservice;
		this.appconstants = appconstants;
	}

	public HPBarAgent(TaskManagementService appservice, AppConstants appconstants, int repeatCount) {
		this.appservice = appservice;
		this.appconstants = appconstants;
	...
	public void run() {
		...
		while (!Thread.currentThread().isInterrupted()) {
			...
			agentlog.setAgent_id(TaskEnum.HP_BARING.getTaskCode());
			try {
				if ("true".equals(appconstants.hp_bar_enabled)) {
				...
				if (repeatCount == 0)
					break;
				Thread.sleep(appconstants.APPLICATION_THREAD_HP_BAR_SLEEP);
