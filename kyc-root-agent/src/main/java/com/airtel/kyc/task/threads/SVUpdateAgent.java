...
public class SVUpdateAgent implements Runnable, DisposableBean {

	private static Logger logger = Logger.getLogger(SVUpdateAgent.class);
	...
	public SVUpdateAgent() {
	}

	public SVUpdateAgent(TaskManagementService appservice, AppConstants appconstants) {
		this.appservice = appservice;
		this.appconstants = appconstants;
	}

	public SVUpdateAgent(TaskManagementService appservice, AppConstants appconstants, int repeatCount) {
		this.appservice = appservice;
		this.appconstants = appconstants;
	...
	public void run() {
		...
		while (!Thread.currentThread().isInterrupted()) {
			...
			agentlog.setAgent_id(TaskEnum.SV_DEMGRAPHIC_UPDATION.getTaskCode());
			try {
				if ("true".equals(appconstants.sv_updation_enabled)) {
				...
				if (repeatCount == 0)
					break;
				Thread.sleep(appconstants.APPLICATION_THREAD_SV_UPDATION_SLEEP);
