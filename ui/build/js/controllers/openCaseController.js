define(['angular'],function(angular){
	...
	.controller('openCaseController', ['$scope','httpService','API','genericService','$filter','$state','genericFactory', function ($scope,httpService,API,genericService,$filter,$state,genericFactory) {
		var url = null;
		var zoomValue = 0.2;
//		alert(genericFactory.decryptionS('roleName'))
		...
		$scope.init = function(){
			...
			if(genericFactory.decryptionS('roleName')=='ROLE_ADMIN' || genericFactory.decryptionS('roleName')=='ROLE_INTERNAL_AUDITOR' || genericFactory.decryptionS('roleName')=='ROLE_REGULATORY' || genericFactory.decryptionS('roleName')=='ROLE_CUSTOMER_SERVICE')
				$scope.searchData = {
						slot: 10, 
						startIndex: 0, 
						actionType: "PENDING_ALL", 
...
// 				$scope.finalSubscriberData.subscriberData.subscriberDetails[0].subscriberIdDetails[0].idImageBackData = $scope.imagesObject.idBack.base64;
// 			}
// 			if($scope.physicalFormDocumentId && $scope.physicalFormDocumentId.value!=null && $scope.imagesObject.physicalFormId && $scope.imagesObject.physicalFormId.base64){
...
// 			$scope.finalSubscriberData.subscriberData.subscriberDetails[0].address.countryId = $scope.addSubscriberObj.subscriberData.subscriberDetails[0].address.countryId.countryId;
// //			console.log(JSON.stringify($scope.finalSubscriberData))
// 			httpService.post(url,$scope.finalSubscriberData).then(function(res){
// 				if(res.data.responseDto.responseCode=="200"){
